...
let input = 
  ...
  // Return record of type "input"
  {
    ...
    es =
      ...
      elementsWithoutCNs |> List.mapi (fun i e -> {e with cn = [i-1; i]})
...
let nodalCoords = input.es |> List.fold (fun state e -> state.Head + e.L :: state) [0.0] |> List.rev |> List.tail
/// Save .png plots
do savePlotsWithTotalFields "c:/Users/Tomas/Temp/" 10 2.0e-4 nodalCoords output.totalFields
